...
import org.webrtc.MediaStream;//顾名思义，这是一个数据流。我们在的图像是变化的，
// 在目前对于影像数据的处理中，都是将每帧图像串成一个流
import org.webrtc.PeerConnection; //我们获取远方数据将会变得和获取本地数据还要简单
import org.webrtc.PeerConnection.IceConnectionState;
import org.webrtc.PeerConnectionFactory;  //WebRTC的所有核心功能都会在基于这个类中
...
import org.webrtc.VideoCapturer;    // 视频捕获
import org.webrtc.VideoRenderer; //设计者也很贴心的为我们提供了界面的展示
import org.webrtc.VideoSource;//VideoSource类提供了对摄像头捕获图像的 开始/暂停 操作，
// 这个接口的好处就是当我们不需要获取图像时，我们可以暂停图像的获取，以此达到省电的作用。
import org.webrtc.VideoTrack; //其实是基于VideoSource包装的，它只是简单的将VideoSoure
// 数据添加到MediaStream中
...
/**
 * PeerConnection的实现，有了这个类才能进行音视频相关数据通讯
...
public class PeerConnectionClient {
    ...
    private PeerConnection peerConnection;  // 音视频通话，包括设备加密和带宽管理。
    ...
    private MediaStream mediaStream;    // 从设备获取数据流，比如说摄像头和麦克风
    ...
    private DataChannel dataChannel;  // p2p通信
    ...
    public static class PeerConnectionParameters {
        public final boolean videoCallEnabled;  // 是否允许视频
        public final boolean loopback;          // 回拨
        public final boolean tracing;
        public final int videoWidth;    // 视频分辨率
        public final int videoHeight;
        public final int videoFps;      // 视频帧率
        public final int videoMaxBitrate; // 视频最大码率
        public final String videoCodec;
        public final boolean videoCodecHwAcceleration;  // 硬件加速
        public final boolean videoFlexfecEnabled;   // FEC冗余
        public final int audioStartBitrate; // 音频相关
    ...
    private void createPeerConnectionInternal(EglBase.Context renderEGLContext) {
        ...
        /**
         * rtcConfig： 有存储 STUN and TURN servers here will enable you to connect, even in hard network scenarios.
         * pcObserver：An instance of your implementation of PeerConnectionObserver.
         * （观察者实例，这个是重点，我们将通过这个类观察连接方的数据）
         */
        ...
        peerConnection.addStream(mediaStream);  // 添加本地数据流到连接中，如果不添加，对方是无法获取我们数据的。
        if (videoCallEnabled) {
            findVideoSender();
        }
        ...
        executor.execute(new Runnable() {
            ...
            public void run() {
                if (peerConnection != null && !isError) {
                    ...
                    isInitiator = true;
                    //在WebRTC有呼叫者和被叫着，这两个函数是初始化呼叫连接时使用的。
            ...
            public void run() {
                if (peerConnection != null && !isError) {
                    if (queuedRemoteCandidates != null) {
                        ...
                        peerConnection.addIceCandidate(candidate);  //创建ICE框架中的连接通道
                    }
                }
            }
        });
    ...
    public void setRemoteDescription(final SessionDescription sdp) {
        executor.execute(new Runnable() {
            ...
            public void run() {
                ...
                peerConnection.setRemoteDescription(sdpObserver, sdpRemote);    // 这是用来发送给对端？
            }
        });
    }
    ...
    /**
     * 观察者（其实就是一个WebRTC提供的回调接口），可以获取我们整个连接的所有状态，
     * 比如：当基于ICE框架连接建立成功、对方有MediaStream数据传送过来或者双方需要协商某些规定时。
     * 我们需要在该接口实现我们接收到数据时所要处理的事情。
     */
            ...
            executor.execute(new Runnable() {
                ...
                public void run() {
                    if (peerConnection != null && !isError) {
                        ...
                        //设置本地和远程 Session Description Protocol (SDP) ，这两个接口配置我们在通信时的相关参数。
                        peerConnection.setLocalDescription(sdpObserver, sdp);   //设置本地sdp，完成设置后onicecandidate事件会调用
                    }
                }
            });
        }
